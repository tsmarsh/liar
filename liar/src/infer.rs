...
impl Inferencer {
    ...
    fn infer_expr(&mut self, expr: &Spanned<Expr>, env: &TypeEnv) -> Ty {
        match &expr.node {
            ...
            // Generated by closure conversion pass - closures are { fn_ptr: ptr, env_ptr: ptr }
            // We treat them as opaque pointers for now
            Expr::ClosureLit { .. } => Ty::Ptr,
            Expr::HeapEnvAlloc { struct_name, .. } | Expr::StackEnvAlloc { struct_name, .. } => {
                Ty::Named(struct_name.clone())
            }
        }
    }
    ...
    fn ast_type_to_ty(ty: &crate::ast::Type) -> Ty {
        ...
        match ty {
            ...
            Type::Ptr => Ty::Ptr,
            // Closure is represented as a struct { fn_ptr: ptr, env_ptr: ptr }
            Type::Closure => Ty::Tuple(vec![Ty::Ptr, Ty::Ptr]),
        }
    }
