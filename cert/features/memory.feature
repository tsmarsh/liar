Feature: Memory Operations (alloca, load, store)
  ...
  Scenario: alloca i32
    Given the expression (define (alloc-i32 ptr) () (ret (alloca i32)))
    When I call alloc-i32
    Then the result is not null

  Scenario: alloca double
    Given the expression (define (alloc-double ptr) () (ret (alloca double)))
    When I call alloc-double
    Then the result is not null

  Scenario: alloca with count
    Given the expression (define (alloc-array ptr) () (ret (alloca i8 (i64 100))))
  ...
  Scenario: store and load i32
    Given the expression (define (store-load-i32 i32) () (let ((p (alloca i32))) (store (i32 42) p) (ret (load i32 p))))
    When I call store-load-i32
    Then the result is (i32 42)

  Scenario: store and load double
    Given the expression (define (store-load-double double) () (let ((p (alloca double))) (store (double 3.14) p) (ret (load double p))))
