Feature: Memory Operations with Pointer Types
  ...
  Scenario: Alloca ptr returns a pointer
    Given the expression (define (alloc-ptr ptr) () (ret (alloca ptr)))
    When I call alloc-ptr
    Then the result is not null

  # store and load ptr
  Scenario: Store and load a pointer
    Given the expression (define (store-load-ptr ptr) ((ptr input)) (let ((p (alloca ptr))) (store input p) (ret (load ptr p))))
    When I call store-load-ptr with (ptr null)
    Then the result is (ptr null)

  Scenario: Store null pointer and load it back
    Given the expression (define (null-roundtrip ptr) () (let ((p (alloca ptr))) (store (ptr null) p) (ret (load ptr p))))
    When I call null-roundtrip
    Then the result is (ptr null)

  # Pointer to pointer operations
  Scenario: Double indirection - pointer to pointer
    Given the expression (define (double-ptr i64) ((i64 val)) (let ((p (alloca i64)) (pp (alloca ptr))) (store val p) (store p pp) (ret (load i64 (load ptr pp)))))
